defmodule Json do
  ...
  defp calcular_porcentaje_bono_individual(json, porcentaje_grupal, niveles) do    
    Enum.reduce(json, %{}, fn jugador, acc -> 
      Map.merge(acc, %{jugador["nombre"] => 
        ((jugador["goles"]/niveles[jugador["nivel"]])+porcentaje_grupal[jugador["equipo"]])/2}) end)    
  end

  defp modificar_json(json, mapa_porcentaje_bono) do
    Enum.map(json, fn jugadores -> Map.replace!(jugadores, "sueldo_completo",
      (jugadores["bono"] * mapa_porcentaje_bono[jugadores["nombre"]]) + jugadores["sueldo"])end)    
  end
